...
ggplot(beer) + 
     geom_histogram(aes(time, fill = as.factor(glass.shape)), binwidth = 1, color = "grey40")  + 
     facet_wrap(~glass.shape, ncol = 1) + 
     theme_bw() 
```
...
```{r, echo=FALSE}
#your code here
ggplot(beer, aes(x = as.factor(glass.shape), y = time)) + 
     geom_boxplot(aes(fill = as.factor(glass.shape))) + 
     theme_bw()
```
...
```{r}
A <- matrix(data = c(2,1,1,  -5,-3,0,   1,1,-1), ncol = 3, byrow = T)
A
...
A - B ... mean = -1, var = 25 (variance doesn't subtract)
...
A+B mean = 9, var = 49 (you add std. dev. and then square it)

formula for variance of sum of two variables is var(A + B) = var(A) + var(B) + 2*cov(A,B)
...
people approached by researchers on UBC campus
<br>
<br>
<br>

b) What is the target population?
unclear -- maybe individuals (students) associated with UBC
<br>
<br>
<br>

c) What is the sampling frame? Does it undercover and/or overcover the target population?
sampling frame == anyone that can be approached.

Overcover -- could be someone who isn't a student
Undercover -- people who refuse or people who don't have class, people who couldn't be reached.
<br>
<br>
<br>
...
Lower right
- We can draw causal inference from it, but we can't generalize to another population (besides
